            ...
            <div style={card}>
                ...
                <div style={{ padding: "14px 20px", borderBottom: `1px solid ${t.borderSecondary}`, display: "flex", alignItems: "center", gap: 10 }}>
                    <div style={{ position: "relative", flex: 1, maxWidth: 420 }}>
                        ...
                        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Filter by action, resource, user, outcome…" style={{ width: "100%", paddingLeft: 30, padding: "8px 12px 8px 30px", background: t.bgInput, border: `1px solid ${t.borderPrimary}`, borderRadius: t.isMono ? 0 : 8, color: t.textPrimary, fontSize: "0.84rem", outline: "none", boxSizing: "border-box", fontFamily: t.fontFamily }} />
                    </div>
                    <span style={{ marginLeft: "auto", fontSize: "0.75rem", color: t.textMuted }}>{total.toLocaleString()} entries</span>
                </div>

                {error && <div style={{ margin: 16, padding: "10px 14px", background: t.statusErrorBg, borderRadius: t.isMono ? 0 : 8, color: t.statusError, fontSize: "0.84rem" }}>{error}</div>}
                                        ...
                                        {isOpen && (
                                            <tr>
                                                <td colSpan={6} style={{ padding: "0 18px 14px", background: t.accentPrimaryMuted }}>
                                                    <div style={{ padding: 16, background: t.bgSecondary, borderRadius: t.isMono ? 0 : 10, border: `1px solid ${t.borderSecondary}` }}>
                                                        ...
                                                        {log.metadata && (
                                                            <pre style={{ padding: 12, background: t.bgTertiary, borderRadius: t.isMono ? 0 : 8, fontSize: "0.75rem", color: t.textSecondary, overflowX: "auto", fontFamily: t.fontMono }}>{JSON.stringify(log.metadata, null, 2)}</pre>
                                                        )}
                                                    </div>
                                                </td>
                                            </tr>
                ...
                {totalPages > 1 && (
                    <div style={{ padding: "12px 20px", borderTop: `1px solid ${t.borderSecondary}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        ...
                        <div style={{ display: "flex", gap: 6 }}>
                            <button disabled={page <= 1} onClick={() => load(page - 1)} style={{ display: "flex", alignItems: "center", padding: "5px 10px", borderRadius: t.isMono ? 0 : 7, border: `1px solid ${t.borderPrimary}`, background: "transparent", color: page <= 1 ? t.textMuted : t.textSecondary, cursor: page <= 1 ? "not-allowed" : "pointer" }}><ChevronLeft style={{ width: 14, height: 14 }} /></button>
                            <button disabled={page >= totalPages} onClick={() => load(page + 1)} style={{ display: "flex", alignItems: "center", padding: "5px 10px", borderRadius: t.isMono ? 0 : 7, border: `1px solid ${t.borderPrimary}`, background: "transparent", color: page >= totalPages ? t.textMuted : t.textSecondary, cursor: page >= totalPages ? "not-allowed" : "pointer" }}><ChevronRight style={{ width: 14, height: 14 }} /></button>
                        </div>
                    </div>
                )}
            </div>

        ...
        <div style={{ padding: "32px 36px", minHeight: "100vh", backgroundColor: t.bgPrimary }}>
            ...
            <div style={{ marginBottom: 28 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    ...
                    <div style={{ display: "flex", gap: 8 }}>
                        <button id="networks-refresh" onClick={loadData} style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 14px", borderRadius: t.isMono ? 0 : 8, border: `1px solid ${t.borderPrimary}`, background: "transparent", color: t.textMuted, fontSize: "0.8rem", cursor: "pointer" }}>
                            <RefreshCw style={{ width: 13, height: 13 }} /> Refresh
                        </button>
                        <button id="networks-create-vpc" onClick={() => setShowCreate(true)} style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 16px", borderRadius: t.isMono ? 0 : 8, border: "none", background: t.accentPrimary, color: t.textInverse, fontSize: "0.8rem", fontWeight: 700, cursor: "pointer" }}>
            ...
            <div style={{ display: "flex", gap: 12, marginBottom: 24 }}>
                {[
                    ...
                    <div key={chip.label} style={{ padding: "8px 18px", borderRadius: t.isMono ? 0 : 8, background: t.bgCard, border: `1px solid ${t.borderPrimary}`, display: "flex", alignItems: "center", gap: 8 }}>
            ...
            {loading ? (
                ...
                <div style={{ ...card, padding: "64px 24px", textAlign: "center" }}>
                    ...
                    <button onClick={() => setShowCreate(true)} style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "10px 22px", borderRadius: t.isMono ? 0 : 8, border: "none", background: t.accentPrimary, color: t.textInverse, fontSize: "0.85rem", fontWeight: 700, cursor: "pointer" }}>
                            ...
                            <div key={vpc.id} style={card}>
                                ...
                                <div style={{ padding: "16px 24px", borderBottom: `1px solid ${t.borderSecondary}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                                    ...
                                    <div style={{ display: "flex", gap: 6 }}>
                                        <button onClick={() => { setShowAssign(vpc.id); setAssignVmId(""); setAssignIp(""); }} style={{ display: "flex", alignItems: "center", gap: 4, padding: "6px 12px", borderRadius: t.isMono ? 0 : 6, border: `1px solid ${t.borderPrimary}`, background: "transparent", color: t.accentPrimary, fontSize: "0.75rem", fontWeight: 600, cursor: "pointer" }}>
                                            <LinkIcon style={{ width: 12, height: 12 }} /> Assign VM
                                        </button>
                                        <button onClick={() => handleDelete(vpc.id)} disabled={deleting === vpc.id} style={{ display: "flex", alignItems: "center", gap: 4, padding: "6px 12px", borderRadius: t.isMono ? 0 : 6, border: `1px solid ${t.statusError}33`, background: t.statusErrorBg, color: t.statusError, fontSize: "0.75rem", fontWeight: 600, cursor: "pointer", opacity: deleting === vpc.id ? 0.5 : 1 }}>
                                ...
                                <div style={{ padding: "12px 24px" }}>
                                    {vpcAssigns.length === 0 ? (
                                        ...
                                        vpcAssigns.map(a => (
                                            <div key={a.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 12px", borderRadius: t.isMono ? 0 : 8, background: t.bgSecondary, marginBottom: 6 }}>
                                                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <div style={{ position: "fixed", inset: 0, zIndex: 9999, background: t.isLight ? "rgba(0,0,0,0.4)" : "rgba(0,0,0,0.7)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
                    <div style={{ ...card, width: "100%", maxWidth: 440, padding: 32 }}>
                        ...
                        <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
                            <button onClick={() => { setShowCreate(false); setCreateErr(""); }} style={{ padding: "9px 18px", borderRadius: t.isMono ? 0 : 8, border: `1px solid ${t.borderPrimary}`, background: "transparent", color: t.textSecondary, cursor: "pointer", fontWeight: 600 }}>Cancel</button>
                            <button onClick={handleCreate} disabled={creating || !createName.trim()} style={{ padding: "9px 20px", borderRadius: t.isMono ? 0 : 8, border: "none", background: t.accentPrimary, color: t.textInverse, fontWeight: 700, cursor: "pointer", opacity: creating || !createName.trim() ? 0.5 : 1 }}>{creating ? "Creating…" : "Create VPC"}</button>
                        </div>
                    </div>
                </div>
            )}
                ...
                <div style={{ position: "fixed", inset: 0, zIndex: 9999, background: t.isLight ? "rgba(0,0,0,0.4)" : "rgba(0,0,0,0.7)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
                    <div style={{ ...card, width: "100%", maxWidth: 440, padding: 32 }}>
                                    ...
                                    <div style={{ display: "flex", gap: 8, marginBottom: assignDhcp ? 0 : 10 }}>
                                        {([[true, "Automatic (DHCP)"], [false, "Manual (static)"]] as const).map(([val, lbl]) => (
                                            <button key={lbl} onClick={() => setAssignDhcp(val)} style={{
                                                flex: 1, padding: "8px", borderRadius: t.isMono ? 0 : 8, cursor: "pointer", fontSize: "0.78rem", fontWeight: 600,
                        ...
                        <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
                            ...
                            {unassignedVMs.length > 0 && (
                                <button onClick={() => handleAssign(showAssign)} disabled={assigning || !assignVmId} style={{ padding: "9px 20px", borderRadius: t.isMono ? 0 : 8, border: "none", background: t.accentPrimary, color: t.textInverse, fontWeight: 700, cursor: "pointer", opacity: assigning || !assignVmId ? 0.5 : 1 }}>{assigning ? "Assigning…" : "Assign"}</button>
                            )}
                        </div>
                    </div>
                </div>
